  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        // Filter to only get offer letters
        const offerLetters = allDocs.filter(doc => doc.documentType === 'offer');
        setDocuments(offerLetters);
        
        // Fetch employee names for each document
        const namesMap: Record<string, string> = {};
        for (const doc of offerLetters) {
    ...
    return (
      <DashboardLayout>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Offer Letters</h1>
    ...
    <DashboardLayout>
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">Offer Letters</h1>
          <Link
            href="/documents/offer-letter/create"
            className="bg-blue-600 text-white px-4 py-2 rounded-md flex items-center gap-2 hover:bg-blue-700"
          >
            <FiPlus /> Create Offer Letter
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        {filteredDocuments.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            {searchTerm ? 'No offer letters match your search' : 'No offer letters found. Create your first offer letter!'}
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredDocuments.map((document) => (
                  <tr key={document.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {document.data?.salary ? formatCurrency(document.data.salary) : 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {deleteConfirm === document.id ? (
                        ...
                        <div className="flex items-center justify-end space-x-2">
                          <Link
                            href={`/documents/offer-letter/${document.id}`}
