...
export default function RelievingLettersPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        // Filter to only get relieving letters
        const relievingLetters = allDocs.filter(doc => doc.documentType === 'relieving');
        setDocuments(relievingLetters);
        
        // Fetch employee names for each document
        const namesMap: Record<string, string> = {};
        for (const doc of relievingLetters) {
        ...
        toast.error('Failed to load relieving letters');
      } finally {
        setLoading(false);
      }
    };
  ...
  const confirmDelete = async (id: string) => {
    try {
      toast.loading('Deleting relieving letter...', { id: 'delete-letter' });
      await deleteDocument(id);
      setDocuments(documents.filter(doc => doc.id !== id));
      setDeleteConfirm(null);
      toast.success('Relieving letter deleted successfully', { id: 'delete-letter' });
    } catch (error) {
      console.error('Error deleting relieving letter:', error);
      toast.error('Failed to delete relieving letter', { id: 'delete-letter' });
    }
  };
    ...
    return (
      <DashboardLayout>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Relieving Letters</h1>
    ...
    <DashboardLayout>
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">Relieving Letters</h1>
          <Link
            href="/documents/relieving-letter/create"
            className="bg-blue-600 text-white px-4 py-2 rounded-md flex items-center gap-2 hover:bg-blue-700"
          >
            <FiPlus /> Create Relieving Letter
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        {filteredDocuments.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            {searchTerm ? 'No relieving letters match your search' : 'No relieving letters found. Create your first relieving letter!'}
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredDocuments.map((document) => (
                  <tr key={document.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {document.data?.lastWorkingDate ? new Date(document.data.lastWorkingDate).toLocaleDateString() : 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {deleteConfirm === document.id ? (
                        ...
                        <div className="flex items-center justify-end space-x-2">
                          <Link
                            href={`/documents/relieving-letter/${document.id}`}
